layui.config({
    ...
    //模块列表
    var ztree = function () {
        var url = '/UserSession/QueryModuleList?isMustEnable=true';
        var zTreeObj;
        var setting = {
    ...
    table.on('checkbox(list)', function (obj) {
        ...
        if (obj.type == "all") {
            var menuList = table.cache["mainList"];
            var hasMenu = menuList != undefined &&
                menuList != null &&
                menuList.length > 0;
            ...
            $.post(url,
                {
                    type: menuType,
                    firstId: id,
                    secIds: menuList.map(function (e) { return e.Id; })
                },
                function (data) {
                    layer.msg(data.Message);
                },
